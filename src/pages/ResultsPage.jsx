...
export default function ResultsPage() {
    ...
    const categoriesWithWinners = allCategories.filter(cat =>
        winners.some(w => w.category_id === cat.id)
    )
    ...
    return (
        <div className="results-page">
            ...
            <div className="results-nav-dots">
                ...
                <button
                    className={`nav-dot ${activeSection === categoriesWithWinners.length ? 'active' : ''}`}
                    onClick={() => scrollToSection(categoriesWithWinners.length)}
                    title="Tổng kết"
                >
            ...
            {/* Hero Section */}
            <section
                className="results-section results-hero"
            ...
            >
                <div className="results-hero-content">
                    ...
                    <p className="hero-subtitle">
                        Những cá nhân và dự án xuất sắc nhất của năm
                    </p>
                    <button
                        className="btn btn-gold scroll-cta"
            ...
            {categoriesWithWinners.map((category, index) => {
                ...
                return (
                    <section
                        key={category.id}
                        className={`results-section winner-section section-${index % 2 === 0 ? 'even' : 'odd'}`}
                    ...
                    >
                        ...
                        <div className="winner-showcase">
                            <div className="winner-avatar-container">
                                <div className="laurel-wreath laurel-left">🌿</div>
                                <img
                                    src={winner.winner?.url_avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(winner.winner?.full_name || 'W')}&background=random&size=200`}
                                    alt={winner.winner?.full_name}
                                    className="winner-avatar"
                                />
                                <div className="laurel-wreath laurel-right">🌿</div>
                            </div>

                            <div className="winner-info">
                                <h3 className="winner-name">{winner.winner?.full_name || winner.winner?.user_name}</h3>
                                {winner.winner?.role && (
                                    <span className="winner-role-badge">{winner.winner.role}</span>
                                )}
                                {winner.notes && (
                                    <p className="winner-description">{winner.notes}</p>
                                )}
                            ...
                            {winner.award_photo_url && (
                                <div className="winner-award-photo-container">
                                    <img
                                        src={winner.award_photo_url}
                                        alt="Award moment"
                                        className="winner-award-photo"
                                    />
                                </div>
                            )}
                        </div>
                        ...
                        <div className="predictor-section">
                            {!predictor && !isLoadingPredictor && (
                                <button
                                    className="predictor-reveal-btn"
                                    onClick={() => loadPredictorForCategory(category.id)}
                                >
                            ...
                            {predictor && predictor.voters && predictor.voters.length > 0 && (
                                <div className="predictor-card">
                                    ...
                                    <div className="predictor-content">
                                        <img
                                            src={predictor.voters[0].voter_avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(predictor.voters[0].voter_name || 'U')}&background=random&size=60`}
                                            alt={predictor.voters[0].voter_name}
                                            className="predictor-avatar"
                                        />
                                        <div className="predictor-info">
                                            ...
                                            <div className="predictor-stats">
                                                ...
                                                {predictor.voters[0].prediction_diff === 0 ? (
                                                    <span className="exact-match"> (Chính xác!)</span>
                                                ) : (
                                                    <span className="diff"> (Sai lệch: {predictor.voters[0].prediction_diff})</span>
                                                )}
                                            </div>
                                        </div>
                                    </div>
                            ...
                            {predictor && (!predictor.voters || predictor.voters.length === 0) && (
                                <div className="no-predictor">
                                    Chưa có ai dự đoán chính xác cho hạng mục này
                                </div>
                            )}
                        </div>
            ...
            {/* Summary Section */}
            <section
                className="results-section results-summary"
                ref={el => sectionsRef.current[categoriesWithWinners.length + 1] = el}
            >
                    ...
                    {categoriesWithWinners.map(category => {
                        ...
                        return (
                            <div key={category.id} className="summary-winner-card">
                                ...
                                <img
                                    src={winner.winner?.url_avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(winner.winner?.full_name || 'W')}&background=random&size=80`}
                                    alt={winner.winner?.full_name}
                                    className="summary-card-avatar"
                                />
                                <div className="summary-card-name">
                                    {winner.winner?.full_name || winner.winner?.user_name}
                                </div>
                                {winner.winner?.role && (
                                    <div className="summary-card-role">{winner.winner.role}</div>
                                )}
                            </div>
                        )
                    })}
                        ...
                        <div className="predictors-list">
                            {topPredictors.map((predictor, index) => (
                                <div key={predictor.session_id} className="predictor-rank-card">
                                    ...
                                    <img
                                        src={predictor.voter_avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(predictor.voter_name || 'U')}&background=random&size=50`}
                                        alt={predictor.voter_name}
                                        className="rank-avatar"
                                    />
                                    <div className="rank-info">
                                        <div className="rank-name">{predictor.voter_name}</div>
                                        <div className="rank-stats">
                                            {predictor.correct_count}/{predictor.total_categories} đúng
                                            ({predictor.accuracy_percent}%)
                                        </div>
                                    </div>
                                </div>
                            ))}
